...
)
{
  ...
  #### Sortie des CA ####
ListeOperations <- distinct(data, format(datedebut, format="%Y-%m-%d")) %>% rename("Date" = names(.[1]))
  CA <- 
    poissons.CAA(Contexte$Nom, periode = "Complet") %>% 
    filter(Date %in% ListeOperations$Date | Ecosysteme == "Référence")
  ...
  ##### Zippage #####
  if(archivage != "Aucun"){
    zip::zipr(zipfile = paste0(Contexte$Nom,".zip"), files = paste0("./",Contexte$Nom))
  }
  if(archivage == "Complet"){
    fs::dir_delete(paste0("./",Contexte$Nom))
  }
